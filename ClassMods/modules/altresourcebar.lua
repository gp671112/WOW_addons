...
function ClassMods.SetupAltResourceBar()
	...
	if ClassMods.db.profile.altresourcebar.enabled then
		...
		if (playerClass == "MONK") and (playerSpec == 1) then -- Brewmaster Monk
			...
			ClassMods.F.AltResourceBar.Stagger:SetValue(UnitStagger("player") )
			...
			ClassMods.F.AltResourceBar.value:SetText(UnitStagger("player"))
			...
			for i=1,numIcons do
				...
				if ((playerClass == "DRUID") and (UnitPowerType("player") == SPELL_POWER_ENERGY)) then
