					...
					for i=1,40 do
						...
						if (self._debuffType == "") and (self._debuffType ~= nil) then -- Enrages have type of an empty string.
							if not self.amShowing then
								...
								self:SetAlpha(1)
								if ClassMods.db.profile.dispel.enablesound then
						...
						elseif (self._debuffType == "Magic") and self._isStealable then
							if not self.amShowing then
								AutoCastShine_AutoCastStart(ClassMods.F.DispelAlert.shine, 1, .5, .5)
								self:SetAlpha(1)
								if ClassMods.db.profile.dispel.enablesound then
	...
	if ( (not lockName) or (lockName == "MOVER_DISPELREMOVABLES") ) then
				...
				function(self, event, ...)
					...
					if (self._subEvent == "SPELL_DISPEL") and (self._sourceGUID == UnitGUID("player") ) and (self._destGUID ~= UnitGUID("pet") ) then
						if (ClassMods.GetChatChan(ClassMods.db.profile.dispel[strlower(ClassMods.GetGroupType() ).."chan"]) ~= "NONE") then
							SendChatMessage("\124cff71d5ff|Hspell:" .. self._spellID .. "\124h[" .. self._spellName .. "]\124h\124r " .. L["removed"] .. " \124cff71d5ff\124Hspell:" .. self._extraSpellID .. "\124h[" .. self._extraSpellName .. "]\124h\124r "
								.. L["from"] .. " " .. self._destName .. ".", ClassMods.GetChatChan(ClassMods.db.profile.dispel[strlower(ClassMods.GetGroupType() ).."chan"]), nil, GetUnitName("player") )
						end
					end
				end)
		end
		...
		ClassMods.F.DispelAlertRemovables:SetScript("OnUpdate",
			function(self, elapsed, ...)
				...
				if (not UnitCanAttack("player", "target") ) or (ClassMods.db.profile.dispel.removablespvponly and (ClassMods.GetGroupType() ~= "ARENA") and (ClassMods.GetGroupType() ~= "BATTLEGROUND") ) then
					for i=1,40 do
						ClassMods.F.DispelAlertRemovables.buffFrames[i].spellID = 0
						ClassMods.F.DispelAlertRemovables.buffFrames[i]:Hide()
					end
					...
					for i=1,40 do
						...
						if (self._debuffType == "Magic") then
							ClassMods.F.DispelAlertRemovables.buffFrames[self._j].spellID = self._spellId
							ClassMods.F.DispelAlertRemovables.buffFrames[self._j].Icon:SetTexture(select(3, GetSpellInfo(self._spellId) ) )
							if ClassMods.db.profile.dispel.removablesenabletexcoords then
								ClassMods.F.DispelAlert.Icon:SetTexCoord(unpack(ClassMods.db.profile.dispel.removablestexcoords) )
							end
