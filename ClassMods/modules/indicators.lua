...
local function checkTargetForIndicator(caller, target)
	if(caller.indicatorTrigger == _Debuff) then
		caller.___name, caller.____, caller.___texture, caller.___stacks, caller.____, caller.___duration, caller.___expires = UnitAura(target, caller.checkID and GetSpellInfo(caller.checkID) or caller.checkName, nil, "PLAYER|HARMFUL")
	elseif(caller.indicatorTrigger == _Buff) then
		caller.___name, caller.____, caller.___texture, caller.___stacks, caller.____, caller.___duration, caller.___expires = UnitAura(target, caller.checkID and GetSpellInfo(caller.checkID) or caller.checkName, nil, "PLAYER|HELPFUL")
	end
