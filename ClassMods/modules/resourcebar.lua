...
function ClassMods.GetBaseGeneration()
	...
	if (select(2, UnitClass("player")) == "DRUID") then
		...
		if UnitAura("player", "Blessing of Elune") then -- increases Lunar Power by 40%
			baseGeneration = baseGeneration + ceil(baseGeneration * .4)
		end
	end
		...
		if ClassMods.db.profile.resourcebar.stacks[playerSpec][i][1] == true then
			if (ClassMods.db.profile.resourcebar.stacks[playerSpec][i][6] == "aura") then
				if (select(2, UnitClass("player")) == "ROGUE") and (playerSpec == 2) then
					...
					checkStacksFunction = function(self) return(select(4, UnitAura(ClassMods.db.profile.resourcebar.stacks[playerSpec][i][3], GetSpellInfo(ClassMods.db.profile.resourcebar.stacks[playerSpec][i][2]), nil,"HELPFUL") ) or select(4, UnitAura(ClassMods.db.profile.resourcebar.stacks[playerSpec][i][3], GetSpellInfo(ClassMods.db.profile.resourcebar.stacks[playerSpec][i][2]), nil,"HARMFUL") ) or 0) end
				end
			...
			numBars = ClassMods.db.profile.resourcebar.stacks[playerSpec][i][4]
		end
	end
