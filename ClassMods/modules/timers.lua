		...
		if ClassMods.db.profile.timers[key].enabled and (#ClassMods.db.profile.timers[key].timers > 0) then
			...
			ClassMods.F.TimerSets[index]:SetScript("OnUpdate",
				function(self, elapsed)
					...
					elseif ClassMods.db.profile.timers[key].mountoverride and (IsMounted() or UnitHasVehicleUI("player") ) and (UnitBuff("player", "Telaari Talbuk") == nil) and (UnitBuff("player", "Frostwolf War Wolf") == nil) and (UnitBuff("player", "Rune of Grasping Earth") == nil) then
