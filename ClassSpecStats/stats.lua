local x = {}
x["Version"] = "|cFFFAFA44裝備屬性選擇建議:|cFF00EA00 2017.3.30|r"

--[[ Deathknight Blood]]
x[250] = "平衡 (推薦): Str > Haste > Vers > Crit > Mast \n 最大輸出: Str > Haste > Crit > Vers > Mast \n 生存能力: Str > Haste > Vers > Mast > Crit "
--[[ Deathknight Frost]]
x[251] = "滅體抹殺: Str > Haste 20% > Crit 25% > Vers > Crit >= Haste 30% > Mast > Haste\n 辛德拉苟莎之息: Str > Crit 25% > Haste 20% > Crit > Vers = Haste = Mast"
--[[ Deathknight Unholy]]
x[252] = "Str > Mast > Haste 接近20% > Crit 或 Vers (視裝等)"

--[[ Druid Balance]]
x[102] = "Haste > Mast > Int > Crit > Vers"
--[[ Druid Feral]]
x[103] =  "Agi > Mast > Crit > Vers > Haste"
--[[ Druid Guardian]]
x[104] = "護甲 > Vers >= Mast > Haste > Agi > Crit"
--[[ Druid Restoration]]
x[105] = "補團隊: Int > Haste > Crit > Mast > Vers \n 補坦/傳奇+: Int > Haste > Mast > Crit > Vers"

--[[ Hunter Beastmaster]]
x[253] = "Mast > Haste > Crit > Vers"
--[[ Hunter Marksmanship]]
x[254] = "Mast >  Crit > Haste > Vers"
--[[ Hunter Survival]]
x[255] = "Haste > Mast(有2件套到12%) > Crit > Vers > Mast(無2件套或過12%)"

--[[ Mage Arcane]]
x[62] = "Int > Haste(到7k) > Vers(到3k) > Crit > Mast > Vers(超過3k) > Haste(超過7k)"
--[[ Mage Fire]]
x[63] = "Crit > Int > Mast = Haste > Vers"
--[[ Mage Fros]]
x[64] = "Crit(到33.34%碎片上限) > Int > Haste > Vers > Mast > Crit(超過33.34%碎片上限)"

--[[ Monk Brewmaster]]
x[268] = "Haste(10～30%) > Crit = Mast = Vers"
--[[ Monk Mistweaver]]
x[270] = "標準: Int > Crit > Vers > Haste > Mast \n 攻擊補血: Int > Vers > Haste >= Crit > Mast \n 傳奇+: Int > Haste = Mast > Vers > Crit"
--[[ Monk Windwalker]]
x[269] = "單目標: Agi > Mast > Crit > Vers > Haste \n 多目標: Agi > Mast > Haste > Crit > Vers"

--[[ Paladin Holy]]
x[65] = "Int > Crit > Vers > Mast > Haste"
--[[ Paladin Protection]]
x[66] = "生存能力: Haste > Vers > Mast > Crit \n 最大輸出: Haste > Crit > Mast > Vers"
--[[ Paladin Retribution]]
x[70] = "Str > Haste > Crit = Vers > Mast"

--[[ Priest Discipline]]
x[256] = "Int > Haste > Crit > Mast > Vers"
--[[ Priest Holy]]
x[257] = "Int > Mast > Crit > Haste > Vers"
--[[ Priest Shadow]]
x[258] = "Haste 到1萬2 > Mast > Crit > Int > Haste > Vers"

--[[ Rogue Assassination]]
x[259] = "標準: Agi > Mast > Crit = Vers > Haste \n 放血: Agi > Vers = Crit > Mast > Haste"
--[[ Rogue Outlaw]]
x[260] = "Agi > Vers > Crit > Mast > Haste"
--[[ Rogue Subtlety]]
x[261] = "Agi > Mast > Vers > Crit > Haste"

--[[ Shaman Elemental]]
x[262] = "冰怒: Int > Crit = Mast > Haste > Vers \n 卓越術: Int > Mast 到86.5% > Crit > Haste > Vers \n 傳奇+/聚雷針: Int > Haste > Crit > Mast > Vers"
--[[ Shaman Enhancement]]
x[263] = "Agi > Mast > Haste > Crit > Vers"
--[[ Shaman Restoration]]
x[264] = "Int > Mast > Crit > Haste > Vers"

--[[ Warlock Affliction]]
x[265] = "Mast > Crit >= Haste > Int > Vers"
--[[ Warlock Demonology]]
x[266] = "Haste > Crit > Mast > Int > Vers"
--[[ Warlock Destruction]]
x[267] = "Int > Haste > Crit > Vers > Mast"

--[[ Warrior Arms]]
x[71] = "Mast > Haste > Vers > Str > Crit"
--[[ Warrior Fury]]
x[72] = "Haste > Mast > Str > Vers > Crit"
--[[ Warrior Protection]]
x[73] = "Str > Haste 到30% > Mast >= Vers > Crit"

--[[ Demon Hunter Havoc]]
x[577] = "Crit > Mast > Agi > Haste > Vers"
--[[ Demon Hunter Vengeance]]
x[581] = "生存能力: Agi > Vers > Mast >= Haste > Crit(T19X4 精通與加速為最佳副屬性) \n 最大輸出: Agi > Crit >= Vers >= Mast >= Haste \n 5人副本: Agi > Mast > Vers > Haste > Crit"
stats_Table = x
