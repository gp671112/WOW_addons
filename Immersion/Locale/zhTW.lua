...
L['Experimental'] = "實驗性功能"
L['Anchor to NPC nameplate'] = "對齊到 NPC 名條"
L['Toggle CVar'] = "開/關 CVar"
L['Show frame on NPC nameplate. This feature requires a CVar to be enabled (nameplateShowFriends).\nClick on the button above to toggle this CVar on/off.\n\nNote that you might have to press Cancel in the interface options for the CVar changes to take effect.'] = "在 NPC 名條的位置顯示框架，這個功能需要啟用 CVar 遊戲參數 (nameplateShowFriends)。\n點一下上方的按鈕來開啟/關閉 CVar 遊戲參數。\n\n請注意，需要按下介面選項視窗的取消按鈕，才能讓 CVar 遊戲參數的更改生效。"
L['Toast when quest is supertracked'] = "再次追蹤時任務顯示通知視窗"
L['When a quest is supertracked (clicked on in the objective tracker, or set automatically by proximity), the quest text will play if nothing else is obstructing it.'] = "再次追蹤的任務 (在任務目標清單中點一下，或靠近時自動追蹤) 會播放任務文字，如果沒有其他東西阻擋的話。"