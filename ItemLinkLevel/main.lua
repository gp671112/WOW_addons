	...
	for itemLink in message:gmatch("|%x+|Hitem:.-|h.-|h|r") do
		...
		if (quality ~= nil and quality >= SavedData.trigger_quality and (itemClassId == LE_ITEM_CLASS_WEAPON or itemClassId == LE_ITEM_CLASS_GEM or itemClassId == LE_ITEM_CLASS_ARMOR)) then
			...
			if (SavedData.show_ilevel and iLevel ~= nil) then 
				local txt = iLevel
				if (ItemHasSockets(itemLink)) then txt = txt .. "帶洞" end
...
local function eventHandler(self, event, ...)
	...
	_G[subtypeCheckBox:GetName().."Text"]:SetText("顯示裝備/武器種類 (布甲, 皮甲...)")
	...
	_G[subtypeShortCheckBox:GetName().."Text"]:SetText("使用簡寫 (布 代表布甲, 皮 代表皮甲)")
	...
	_G[equipLocCheckbox:GetName().."Text"]:SetText("顯示裝備部位 (頭部, 飾品...)")
	...
	_G[iLevelCheckbox:GetName().."Text"]:SetText("顯示物品等級")
	...
	_G[triggerLootsCheckbox:GetName().."Text"]:SetText("啟用在拾取物品 (需要重新載入)")
	...
	_G[triggerChatCheckbox:GetName().."Text"]:SetText("啟用在聊天訊息連結 (需要重新載入)")
	...
	triggerQualityLabel:SetText("顯示的最低品質")
