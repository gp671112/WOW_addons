...
L["Max champions"] = "最大追隨者數"
L["Use at most this many champions"] = "最多使用的追隨者"
L["Don't use troops"] = "不使用部隊"
L["Only use champions even if troops are available"] = "即使部隊可用也只使用追隨者"
L["Ignore busy followers"] = "忽略忙碌的追隨者"
L["When no free followers are available shows empty follower"] = "如果沒有空閒的追隨者可用顯示為空"
L["Ignore inactive followers"] = "忽略停用的追隨者"
L["If not checked, inactive followers are used as last chance"] = "勾選後，停止任用的追隨者將不會計算到機率中"
L["Artifact shown value is the base value without considering knowledge multiplier"] = "神兵顯示值為不考慮知識乘數的基礎值"
L["Not enough troops, raise maximum champions' number"] = "沒有足夠的部隊，提高最大追隨者的數量"
L["You have no troops"] = "你沒有部隊了"
L["Unable to fill missions. Check your switches"] = "無法排組任務，檢查你的開關"
L['but using troops with just one durability left'] = "但使用僅剩一次耐久度的部隊"
return
end
