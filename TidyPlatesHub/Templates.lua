...
local function PasteSettings(panel)
	...
	if IsShiftKeyDown() then
		...
		print(orange.."已貼上設定，從 "..yellow..panel.name..orange.." 剪貼簿。")
	else
		cacheName = "GlobalClipboard"
		print(orange.."已從剪貼簿貼上設定。")
	end
...
local function CopySettings(panel)
	...
	if IsShiftKeyDown() then
		cacheName = panel.objectName
		print(blue.."已將設定複製到 "..yellow..panel.name..blue.." 剪貼簿。"..yellow.." 要使用這些設定值，按住 'Shift' 不放然後按下 '貼上' 按鈕。")
	else
		cacheName = "GlobalClipboard"
		print(blue.."已將設定複製到剪貼簿。")
	end
...
local function ResetSettings(panel)
	if IsShiftKeyDown() then
		...
		OnPanelItemChange(panel)
		print(yellow.."正在重置 "..orange..panel.name..yellow.." 的設定為預設值")
		print(yellow.."按住 "..blue.."Shift"..yellow.." 不放然後按下"..red.."重置"..yellow.." 將會清除所有已經儲存的設定、快取資料，然後重新載入介面。")
	end
end
...
local function CreateInterfacePanel( objectName, panelTitle, parentTitle)
	...
	PasteThemeDataButton.tooltipText = "將你的設定從快取貼入。'Shift'-滑鼠左鍵點擊使用面板專用快取"
	...
	CopyThemeDataButton.tooltipText = "將你的設定複製到快取。'Shift'-滑鼠左鍵點擊使用面板專用快取"
	...
	ReloadThemeDataButton.tooltipText = "重置設定成預設值。按住 'Shift' 同時會清除已儲存的單位資料，並且重新啟動插件。"
...
--PanelHelpers.HideDropdownMenu
	...
	ClosePanel = function(self)
        ...
        local function OpenTidyPlatesConfig()
            InterfaceOptionsFrame_OpenToCategory("Tidy 血條")
			InterfaceOptionsFrame_OpenToCategory("Tidy 血條")
        end
        ...
        local RefreshPanel = function(self)
            ...
            if TidyPlates.ActiveThemeTable.OnUpdate ~= TidyPlatesHubFunctions.OnUpdate then
                ...
                panel.Warnings.Text:SetText("看來，你所使用的主題和套件中心不相容。")
                panel.Warnings.Text:SetTextColor(1, 1, 1)
                panel.Warnings.Text:SetFont(font, 18)

                WarningFixButton:SetText("更換主題...")
