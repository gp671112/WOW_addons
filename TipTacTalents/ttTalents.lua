...
function ttt:InitiateInspectRequest(unit,record)
	...
	if (CanInspect(unit)) and (not IsInspectFrameOpen()) then
		local cfg = TipTac_Config or TTT_DefConfig;
		local freq = (cfg.inspectFreq or TTT_DefConfig.inspectFreq);	-- remove this extra fallback once a new main TipTac is released
		local delay = (cfg.inspectDelay or TTT_DefConfig.inspectDelay);
